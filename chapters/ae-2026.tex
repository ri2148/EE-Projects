...
Substiuting results \eqref{eq:Matrices_1} and \eqref{eq:Matrices_inverse} in \eqref{eq:transpose}, we get:
\begin{align}
...
\begin{align}
\myvec{ a & c \\ b & d } \myvec{ 1 \\ -1 } &= \myvec{ a - c \\ b - d } = \myvec{ k' \\ -k' } = k' \myvec{ 1 \\ -1 }
\end{align}
From \eqref{eq:eigenvalues}, $\myvec{ 1 \\ -1 }$ is an eigenvector and $d - b$ is the $2^{\text{nd}}$ eigenvalue.
...
\begin{align}
	y_{70}^{\prime}=\frac{y_{70}-y_{69}}{h} \approx 0.145
\end{align}
\begin{lstlisting}[frame=single, basicstyle=\rmfamily, escapeinside={(*@}{@*)}]
