...
\noindent Find the value of $P\brak{AB'}$ \\
\solution \\ 
...
\noindent Consider
\begin{align}
c_k = f_0 \int_{-\frac{1}{2f_0}}^{\frac{1}{2f_0}} x\brak{t} e^{-j 2\pi k f_0 t} \, dt
\end{align}
...
\noindent Substituting $c_k$ into the following synthesis equation reconstructs the time-domain signal as an infinite sum of complex exponentials:
\begin{align}
\tilde{x}(t) = \sum_{k=-\infty}^{\infty} \left( \frac{\sin(k\pi/2)}{k\pi} \right) e^{j \frac{k\pi t}{2}}
\end{align}

\begin{align}
\tilde{x}(t) = \frac{1}{2} + \frac{2}{\pi} \cos \left( \frac{\pi t}{2} \right) - \frac{2}{3\pi} \cos \left( \frac{3\pi t}{2} \right) + \frac{2}{5\pi} \cos \left( \frac{5\pi t}{2} \right) - \dots
\end{align}
\begin{figure}[H]
    \centering
    ...
    \long\def\@makecaption#1#2{%
      ...
      \vskip\belowcaptionskip}
    \makeatother
    \caption{REpresentation of signal}
    \label{fig:7}
\end{figure}
...
\noindent Solution with Euler's Method: \\
...
\end{align}
\noindent Substituting the values of $M$ and $k_{eq}$, we get:
\begin{align}
\frac{d^2 x}{d t^2} + 10^6 x = 0
\end{align}
\noindent Let the initial displacement of the system be $x_0$ and inital velocity be $v_0$. Taking Laplace Transform on both sides, and using the operational property from \eqref{eq:Laplace_2}, we get:
\begin{align}
s^2 X(s) - s x_0 - v_0 + 10^6 X(s) = 0
\end{align}
\begin{align}
X(s) = \frac{s x_0 + v_0}{s^2 + 10^6} 
\end{align}
\begin{align}
X(s) = x_0 \brak{\frac{s}{s^2 + 1000^2}} + \frac{v_0}{1000} \brak{\frac{1000}{s^2 + 1000^2}}
\end{align} 
