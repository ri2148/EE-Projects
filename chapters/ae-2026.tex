...
\end{align}
\noindent\framebox[\dimexpr\textwidth-2\fboxsep-2\fboxrule\relax][c]{codes/Circle.py}
\begin{figure}[H]
    \centering
...
We will use the result \eqref{eq:Fourier_1}. Considering $f_0 = \frac{1}{4}$, $T_0 = 4$, and the integration interval is $\brak{-\frac{1}{2f_0}, \frac{1}{2f_0}} = \brak{-2, 2}$:
...
For $k = 0$:
\begin{align}
c_0 &= \frac{1}{4} \int_{-1}^{1} 1 \, dt = \frac{1}{4} \brak{1 - \brak{-1}} = \frac{1}{2} 
\end{align}

For $k \neq 0$:
\begin{align}
c_k &= \frac{1}{4} \left[ \frac{e^{-j \frac{\pi k t}{2}}}{-j \frac{\pi k}{2}} \right]_{-1}^{1}
\end{align}
\begin{align}
= \frac{1}{-j 2\pi k} \brak{e^{-j \frac{\pi k}{2}} - e^{j \frac{\pi k}{2}}} 
\end{align}
\begin{align}
= \frac{-2j \sin\brak{\frac{\pi k}{2}}}{-j 2\pi k} = \frac{\sin\brak{\frac{\pi k}{2}}}{\pi k} 
\end{align}
\begin{figure}[H]
    \centering
...
\noindent We will solve it with an example: \\
...
\end{align}
\noindent Finding the norms:
\begin{align}
\norm{\vec{r}} = \sqrt{8000^2 + 9000^2} = \sqrt{64 \times 10^6 + 81 \times 10^6} = \sqrt{145 \times 10^6} \approx 12041.59 \text{ km}
\end{align}
\begin{align}
\norm{\vec{v}} = \sqrt{-6^2 + 6^2} = \sqrt{36 + 36} = \sqrt{72} \text{ (km/s)}
\end{align}
\begin{align}
\norm{\vec{v}}^2=72 \text{ (km/s)}^2
\end{align}
...
\end{align}
The polar trajectory of a satellite with eccentricity vector $\mathbf{e}$ and semi-latus rectum $p_{\text{orb}}$ is given by:
\begin{align}
r = \frac{p_{\text{orb}}}{1 + e \cos \theta}
\end{align}

In vector form, $e \cos \theta = \frac{\mathbf{e}^\top \mathbf{x}}{r}$, where $\mathbf{x} = \begin{pmatrix} p \\ q \end{pmatrix}$.
\begin{align}
...
\end{itemize}
\noindent From result \eqref{eq:mat_det_lemma}, we have:
\begin{align}
\mydet{\mathbf{V}}=1-\mathbf{e^Te}=1-e^2
\end{align}
...
\mydet{\mathbf{V}}<0 
\end{align}
\noindent From \ref{item:Hyperbola}, we can conclude that the trajectory of the satellite is a Hyperbola.

\noindent\framebox[\textwidth][c]{codes/Satellite.py}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.60\linewidth]{figs/Satellite.png}
	\makeatletter
    \long\def\@makecaption#1#2{%
      \vskip\abovecaptionskip
      \begin{center}
...
\begin{align}
	y_{70}'=0.145
\end{align}
\noindent\framebox[\dimexpr\textwidth-2\fboxsep-2\fboxrule\relax][c]{codes/Euler.py}
\begin{figure}[H]
    \centering
